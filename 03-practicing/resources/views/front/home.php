...
?>
<?php
$latest = db_frist('news', 'order by id desc');
?>
...
');
$news2 = db_frist('news', '
    JOIN categories ON news.category_id = categories.id where news.category_id!="' . $news1['cat_id'] . '"
    ORDER BY news.id DESC
', '
    categories.name as cat_name,
    categories.id as cat_id,
    ...
    <div class="col-md-6">
      <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
        <div class="col p-4 d-flex flex-column position-static">
          ...
          <a href="<?= url('news?category_id=' . $news2['cat_id'] . '&id=' . $news2['id']) ?>" class="icon-link gap-1 icon-link-hover stretched-link">
            <?= trans('main.read_more') ?>
            <svg class="bi">
              <use xlink:href="#chevron-right" />
            </svg>
      ...
      <div>
        ...
        <ul class="list-unstyled">
          ...
          while ($news = mysqli_fetch_assoc($latest_news['query'])): ?>
            <li>
              <a class="d-flex flex-column flex-lg-row gap-3 align-items-start align-items-lg-center py-3 link-body-emphasis text-decoration-none border-top" href="<?= url('news?category_id=' . $news['category_id'] . '&id=' . $news['id']) ?>">
                <div class="bd-placeholder-img" width="100%" height="96">
                  <?php
                  if (!empty($news['image'])) {
